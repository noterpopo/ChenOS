...
int32_t pcb_fd_install(int32_t global_fd_idx) {
    ...
    uint8_t local_fd_idex = 3;
    while (local_fd_idex < MAX_FILES_OPEN_PER_PROC) {
        if (cur->fd_table[local_fd_idex] == -1) {
            cur->fd_table[local_fd_idex] = global_fd_idx;
            break;
        }
        local_fd_idex++;
    }
    if (local_fd_idex == MAX_FILES_OPEN_PER_PROC) {
        printk("exceed max open files_pre_proc\n");
        return -1;
    }
    return local_fd_idex;
}
...
int32_t file_create(struct dir* parent_dir, char* fileanme, uint8_t flag) {
    ...
    create_dir_entry(fileanme, inode_no, FT_REGULAR, &new_dir_entry);
    if (!sync_dir_entry(parent_dir, &new_dir_entry, io_buf)) {
        printk("sync dir_entry to disk\n");
        rollback_step = 3;
        goto rollback;
    }
    ...
    if (flag & O_WRONLY || flag & O_RDWR) {
        ...
        if (!(*write_deny)) {
            *write_deny = true;
            ...
            intr_set_status(old_status);
            printk("file cannot be write now\n");
            return -1;
        }
    }
    return pcb_fd_install(fd_idx);
