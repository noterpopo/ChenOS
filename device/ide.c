...
struct boot_sector {
    uint8_t other[446];
    struct partition_table_entry partiton_table[4];
    uint16_t signature;
} __attribute__ ((packed));
...
static void read_from_sector(struct disk* hd, void* buf, uint8_t sec_cnt) {
    ...
    if (sec_cnt == 0) {
        size_in_bytes = 256 * 512; // 进位丢掉1变为0
    } else {
...
void intr_hd_handler(uint8_t irq_no) {
    ASSERT(irq_no == 0x76 || irq_no == 0x77);
    uint8_t ch_no = irq_no - 0x76;
...
static void partition_scan(struct disk* hd, uint32_t ext_lba) {
    ...
    uint8_t patr_idx = 0;
    struct partition_table_entry* p = bs->partiton_table;
    while (patr_idx++ < 4) {
        if (p->fs_type == 0x5) { //扩展分区
            if (ext_lab_base != 0) {
                partition_scan(hd, p->start_lba + ext_lab_base);
            } else {
                ext_lab_base = p->start_lba;
