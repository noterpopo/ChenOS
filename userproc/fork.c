...
static int32_t copy_pcb_vaddrbitmap_stack_0(struct task_struct* child_thread, struct task_struct* parent_thread) {
    ...
    memcpy(vaddr_bitmap, child_thread->userproc_vaddr.vaddr_bitmap.bits, bitmap_pg_cnt * PG_SIZE);
    child_thread->userproc_vaddr.vaddr_bitmap.bits = vaddr_bitmap;
...
static void copy_body_stack3(struct task_struct* child_thread, struct task_struct* parent_thread, void* buf_page) {
    ...
    while (idx_byte < btmp_bytes_len) {
        if (vaddr_btmp[idx_byte]) {
            idx_bit = 0;
            while (idx_bit < 8) {
                if ((BIT_MASK << idx_bit) & vaddr_btmp[idx_byte]) {
...
static int32_t build_child_stack(struct task_struct* child_thread) {
    ...
    int32_t* edi_ptr_in_thread_stack = (uint32_t*)intr_0_stack - 3;
    int32_t* ebx_ptr_in_thread_stack = (uint32_t*)intr_0_stack - 4;
    uint32_t* ebp_prt_in_thread_stack = (uint32_t*)intr_0_stack - 5;
    *ret_addr_in_thread_stack = (uint32_t)intr_exit;
    *ebp_prt_in_thread_stack = *ebx_ptr_in_thread_stack = *edi_ptr_in_thread_stack = *esi_ptr_in_thread_stack = 0;
    child_thread->self_kstack = ebp_prt_in_thread_stack;
    return 0;
}
...
static int32_t copy_process(struct task_struct* child_thread, struct task_struct* parent_thread) {
    ...
    if (copy_pcb_vaddrbitmap_stack_0(child_thread, parent_thread) == -1) {
        return -1;
    }
...
pid_t sys_fork(void) {
    struct task_struct* parent = (struct task_struct*)running_thread();
